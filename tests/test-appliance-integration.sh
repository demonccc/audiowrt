...
if grep -qx 'audiowrt-storage' "$repo_root/config/packages.add"; then
    echo 'ERROR: storage/extroot must remain optional in the 8 MB functional baseline.' >&2
    exit 1
fi
grep -qx 'audiowrt-bluetooth' "$repo_root/config/packages.add"
grep -qx 'audiowrt-usb-audio' "$repo_root/config/packages.add"

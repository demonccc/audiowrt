...
grep -Fq 'target_roots+=("${spec%%|*}")' "$build_script"
grep -Fq '"${target_roots[@]}"' "$build_script"
grep -Fq 'target_package_seen["${spec%%|*}"]=1' "$build_script"
...
grep -Fq 'target_package_config_args+=("CONFIG_PACKAGE_kmod-bluetooth=n")' "$build_script"
grep -Fq 'target_package_config_args+=("CONFIG_PACKAGE_kmod-audiowrt-bluetooth=n")' "$build_script"

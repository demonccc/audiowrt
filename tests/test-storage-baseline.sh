...
# Storage uses official OpenWrt packages, never the removed AudioWRT wrappers.
...
! grep -q '^audiowrt-storage' "$repo_root/config/build/package-build-targets"
grep -q '^audiowrt-config|' "$repo_root/config/build/package-build-targets"
grep -q '^audiowrt-identity|' "$repo_root/config/build/package-build-targets"

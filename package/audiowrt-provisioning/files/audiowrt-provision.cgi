...
ssid=''; key=''; encryption='sae-mixed'; old_ifs="$IFS"; IFS='&'
for pair in $body; do
	field="${pair%%=*}"; value="${pair#*=}"; decoded="$(uhttpd -d "$value" 2>/dev/null || true)"
	case "$field" in ssid) ssid="$decoded" ;; key) key="$decoded" ;; encryption) encryption="$decoded" ;; esac
done
IFS="$old_ifs"
